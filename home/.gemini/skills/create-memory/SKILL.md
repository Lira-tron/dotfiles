...
## ⚠️ CRITICAL: Gemini Knowledge Base Integration

**This uses Gemini's native knowledge base:**
- Files are stored as markdown in `~/.gemini/knowledge/memory/`
- **Global knowledge base**: Accessible across ALL workspaces and projects
- Automatically indexed and searchable via `/search` in Gemini from any workspace
- No manual indexing needed - Gemini handles it automatically
- Search and list functionality comes from Gemini's core features
...
- **Knowledge Base**: `~/.gemini/knowledge/memory/{project}/{YYYY}/{MM}/{topic}.md`
- **Index**: `~/.gemini/knowledge/memory/index.md`
...
**Path format:** `~/.gemini/knowledge/memory/{project}/{YYYY}/{MM}/{topic}.md`
...
**Index path:**
`~/.gemini/knowledge/memory/index.md`
...
This entry is now searchable via /search in Gemini.
```
...
{
  "resources": [
    "skill://.gemini/skills/create-memory/SKILL.md"
  ]
}
```
...
gemini "Store the GraphQL implementation approach we just discussed"

# Store with specific topic
gemini "Store decision to use event-driven architecture for the payment service"

# Store from a document
gemini "Store this design doc to memory" # (with design.md open or referenced)

# Store from a URL
gemini "Store this design doc https://example.com/design to memory"

# Store debugging solution
gemini "Store the fix for the memory leak in user service"
```
...
- **Automatic Indexing**: Gemini automatically indexes all markdown files in `.gemini/knowledge/`
- **Native Search**: Use `/search` in Gemini to find stored entries from any workspace
- **No Manual Commands**: Just save files - Gemini handles indexing
