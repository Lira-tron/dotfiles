---
name: sync-steering
description: Sync existing steering documents from workspace .gemini/steering/ to knowledge base with Context sections. Use when steering docs already exist and need syncing to knowledge base.
...
## Overview
Syncs existing steering documents from the local workspace to the knowledge base, adding Context sections for searchability and session recovery. Use this when steering documents already exist in `.gemini/steering/` and need to be synced to the knowledge base.
...
- **Any other .md files** in `.gemini/steering/`

## File Locations
Documents are synced FROM and TO:
- **Source (Workspace)**: `.gemini/steering/` (local to current project)
- **Destination (Knowledge Base)**: `~/.gemini/knowledge/steering/{workspace-name}/`
...
- Locate `.gemini/steering/` directory
- List all steering documents present

### Step 2: Check Existing Steering Documents
Scan `.gemini/steering/` for existing documents:
- List all `.md` files
...
3. **Save to knowledge base**: `~/.gemini/knowledge/steering/{workspace-name}/`
...
- Source: .gemini/steering/{filename}
...
**Sync Process:**
1. Detect changes in local `.gemini/steering/` files
...
- Structural changes
- New files added to `.gemini/steering/`
...
Locations:
📁 Source: .gemini/steering/ (clean, no Context sections)
...
These documents are now searchable via /search in Gemini.
```
...
{
  "resources": [
    "skill://.gemini/skills/sync-steering/SKILL.md"
  ]
}
```
...
2. Locates all steering documents in `.gemini/steering/`
...
- **Handles All Files**: Syncs any `.md` file in `.gemini/steering/`, not just product/tech/structure
