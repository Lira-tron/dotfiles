...
## Overview
Generates foundational steering documents for the current workspace and saves them to both local `.gemini/steering/` and the knowledge base for searchability.
...
- **Workspace**: `.gemini/steering/` (local to current project)
- **Knowledge Base**: `~/.gemini/knowledge/steering/{workspace-name}/`
...
- `.gemini/steering/product.md`
- `.gemini/steering/tech.md`
- `.gemini/steering/structure.md`
...
Locations:
📁 Workspace: .gemini/steering/ (clean, no Context sections)
...
{
  "resources": [
    "skill://.gemini/skills/generate-steering/SKILL.md"
  ]
}
```
...
gemini "Generate steering documents"

# Or with explicit instruction
gemini "Generate the project documentation"
```
